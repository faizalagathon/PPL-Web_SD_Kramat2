...
function halAktif($hal)
{
  global $indexTerakhir;
  return array_search($hal, explode('?', explode('/', $_SERVER['REQUEST_URI'])[$indexTerakhir]));
}
$dir = '../';
...
// var_dump(explode('/', $_SERVER['REQUEST_URI']), $dir, explode('?', explode('/', $_SERVER['REQUEST_URI'])[$indexTerakhir]), array_search('galeri.php', explode('?', explode('/', $_SERVER['REQUEST_URI'])[$indexTerakhir])));
?>
